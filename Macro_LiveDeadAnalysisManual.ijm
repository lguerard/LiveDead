...
Folder = getDirectory("Choose the folder where your images are located");
	...
	MinSizeObject = getNumber("What is the minimum size of the object you want to measure (in pixel unit)?", 3);

	//Name of the output text file
	TextName = trueYear+trueMonth+trueDay+"fixedSize"+MinSizeObject+".txt";
	...
	for (i=0; i<NrOfFiles; i++) {
		...
		{ 
			...
			{
				test = 1;
				print("Treating "+ShortFileNameWithoutChannelInfo+" green image");
				//Threshold to the corresponding values
				setAutoThreshold("Yen dark");
				setThreshold(33,255);
				//Get the different objects in the file
				run("Analyze Particles...", "size="+MinSizeObject+"-Infinity pixel show=Nothing display add clear");
				ColorArea = 0;
			...
			{
				test = 2;
				print("Treating "+ShortFileNameWithoutChannelInfo+" red image");
				//Threshold to the corresponding values
				setAutoThreshold("Yen dark");
				setThreshold(19,255);
				//Get the different objects in the file
				run("Analyze Particles...", "size="+MinSizeObject+"-Infinity pixel show=Nothing display add");
				ColorArea = 0;
				//Loop through the results
			...
			if(File.exists(Folder+File.separator+ShortFileNameWithoutChannelInfo+"_c1.tif") && File.exists(Folder+File.separator+ShortFileNameWithoutChannelInfo+"_c2.tif") && test==2)
			{
				...
				run("Flatten");
				saveAs("tiff",Output_Folder+File.separator+"ResultImages"+File.separator+trueYear+trueMonth+trueDay+"FixedThreshold_Size"+MinSizeObject+ShortFileNameWithoutChannelInfo);
